...
def default_libtype(search_query: dict) -> str | None:
    """Return to a sane libtype if not explicitly provided."""
...
def find_exact_match(results: list, search_query: dict) -> PlexObject:
    """Return one search match."""
...
def search_media(
    ...
    **kwargs,
    ...
    if len(results) > 1:
        if allow_multiple:
            return results
        return find_exact_match(results, search_query)
